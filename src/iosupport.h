...
potrzebne makra/funkcje io:
	...
	=odczytaj wartoæ ADC (ADCH)				c			r16				|


= sta³a definicja (sta³y adres, zostawiæ)
? ró¿na iloæ elementów do ustawienia (setup)
...
+sprawdzanie trybu (tryb, iloæ kana³ów)
	mode (1, 2, 3)
	outputtype (1, 2)
...
+sprawdzanie czy piny nie pokrywaj¹ siê ze sob¹
	...
	0		1		sprawdzanie poprawnoci z innymi pinami
...
+deklarowanie/w³¹czanie ustawnieñ dla setapu
	...
	1		0		nic nierób
	1		1		w³¹cz ustawienie w setapie (jeli to mo¿liwe)	???
...
(szkice)
ustawienia fal (dane wejciowe):
-tryb ustawienia fali (faza, czêstotliwoæ, g³onoæ)
-numer fali
-warto do zapisania
void ustawfale([numer fali], [tryb], [wartoæ])

???adres pamiêci liczony w assemblerze lub liczony w c (sta³a/zmienna wartoæ)???
...
-tryb (sprawdzanie trybu):
 ...
 -g³onoæ
 -czekanie
-zapisanie kolejnych wartoci do ram
  

organizacja pamiêci:
[tryb - 2B (ew. 3B) | numer fali/iloæ bajtów czekania - 6B (ew. 5B)] [wartoæ (hb)] [wartoæ (lb)]
